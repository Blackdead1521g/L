...
int main(void)
{
    setup(); //Llamamos el setup

    while(1){
        valor = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4); //Leemos el estado del boton
        if(valor == 0){
            antirebote = 1;
        }
        if(valor == 0 && antirebote == 1){
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, GPIO_PIN_3); //Enciende el led verde
            delay(4000); //Delay de 4s
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0); //Apaga el led verde
            delay(200); //200ms
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, GPIO_PIN_3); //Enciende el led verde
            delay(200); //200ms
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0); //Apaga el led verde
            ...
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3|GPIO_PIN_1 ,GPIO_PIN_3|GPIO_PIN_1); //Enciende el led verde y el rojo para hacer el amarillo
            delay(4000); //Delay de 4s
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3|GPIO_PIN_1, 0); //Apaga el led amarillo
            delay(500); //500ms
...
void setup(void){
    SysCtlClockSet(SYSCTL_SYSDIV_2_5|SYSCTL_USE_PLL|SYSCTL_OSC_MAIN|SYSCTL_XTAL_16MHZ); //Reloj del sistema 80Hz
